...
function frame(document: Map<unknown, unknown>, version = 2): string {
  const body = `spectrl.v${version}.${b64urlEncode(cborEncode(document))}`
...
test("spectrum user parameters use key 7 and scan parameters retain key 2", () => {
  ...
  for (const lossless of [false, true]) {
    ...
    const document = cborDecode(b64urlDecode(token.split(".")[2]!)) as Map<number, unknown>
...
test("only the current format is accepted", () => {
  for (const version of [0, 1, 3, 99]) {
    assert.throws(() => decodeToken(frame(new Map([[0, 0]]), version)), /Not a spectrl.v2 token/)
  }
})
