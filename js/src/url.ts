...
/** Extract a spectrl.v2 token from a fragment, query string, or data: URI. */
export function extractToken(urlOrUri: string): string {
  ...
  throw new Error(`No spectrl.v2 token found in: ${JSON.stringify(urlOrUri)}`);
}
