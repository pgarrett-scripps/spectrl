...
const MAGIC_PREFIX = "spectrl.v3.";
const DATA_URI_PREFIX = "data:application/vnd.spectrl;v=3,";
...
/** Extract a spectrl.v3 token from a fragment, query string, or data: URI. */
export function extractToken(urlOrUri: string): string {
  ...
  throw new Error(`No spectrl.v3 token found in: ${JSON.stringify(urlOrUri)}`);
}
