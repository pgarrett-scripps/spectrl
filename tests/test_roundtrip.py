...
def test_charge_array_with_negative_values_lossy_roundtrips():
    """Lossy encoding of a charge array with negative sentinels must not abort
    (PIC cannot encode negatives); it falls back to lossless and round-trips
    exactly."""
    n = 3
    mz = np.array([100.0, 200.0, 300.0])
    intensity = np.array([1000.0, 5000.0, 2500.0])
    charge = np.array([1.0, 2.0, -1.0])  # -1 = unassigned/singleton sentinel
    spec = InlineSpectrum(default_array_length=n, mz=mz, intensity=intensity, charge=charge)
...
def test_negative_intensity_lossy_roundtrips_exactly():
    """Negative intensities (e.g. baseline-subtracted data) cannot go through
    slof, so encoding falls back to lossless and round-trips exactly."""
    mz = np.array([100.0, 200.0, 300.0])
    intensity = np.array([-0.5, 1e4, -3.0])
    spec = InlineSpectrum(default_array_length=3, mz=mz, intensity=intensity)
    decoded = decode_token(encode_spectrum(spec))  # default: lossy
    np.testing.assert_array_equal(decoded.intensity, intensity)


def test_negative_ion_mobility_lossy_roundtrips_exactly():
    mz = np.array([100.0, 200.0])
    intensity = np.array([1e3, 2e3])
    im = np.array([-0.001, 0.9])
    spec = InlineSpectrum(
